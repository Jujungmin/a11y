...
function modalOpen($modal) {
    ...
    $modal = $('.' + $modal);
    $modal.css({display: 'block'}).attr('aria-modal', 'true').attr('role', 'dialog').removeAttr('aria-hidden');
...
function modalClose($modal) {
    ...
    if($modal === 'all') {
        $('.modal').css({display: 'none'}).attr('arai-hidden', 'true').removeAttr('aria-modal').removeAttr('role');
    } else {
        $($modal).closest('.modal').css({display: 'none'}).attr('arai-hidden', 'true').removeAttr('aria-modal').removeAttr('role');
    }
...
$(function() {
    // paging 현재 페이지 나타내기
    $('.paging').find('a').hasClass('active') ? $('.paging a.active').attr('aria-current', 'true') : $('.paging a').removeAttr('aria-current');
    
    // input text 모두 지우기
    $('.button-delete').on('click', (e)=> {
        $(e.target).siblings('input').val('cmd');
    })
    // 체크박스 모두 선택하기
